...
@Component
class ImageBatchDataLoader(private val imageRegistryService: ImageRegistryService) : GoboDataLoader<ImageTag, DataFetcherResult<Image?>>() {
    override suspend fun getByKeys(keys: Set<ImageTag>, ctx: GoboGraphQLContext): Map<ImageTag, DataFetcherResult<Image?>> {
        return keys.associateWith { key ->
            ...
            runCatching {
                ...
                DataFetcherResult.newResult<Image?>().error(GraphQLExceptionWrapper(it)).build()
            }.getOrThrow()
        }
    }
}
